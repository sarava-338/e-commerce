...
public class ECommerceApplicationTests {
	...
	@Autowired
	CategoryRepo ProblemStatementcategoryRepo;
	@Autowired
	UserRepo userRepo;
	@Autowired
	...
	public void sellerCheckAddedNewProduct() throws Exception {
		Assert.notNull(createdURI, "uri is null");
		Assert.isTrue(!createdURI.isEmpty(), "uri is blank");
		assertEquals(new URL(createdURI).getPath(), "/api/auth/seller/product/3");
